<template>
    <div class="container">
        ...
        <div class="main layoutWrap clearfix">
           <template>
                <Breadcrumb firstName="我的消息" secondName="商会消息" firstLink="/" ></Breadcrumb>
            </template> 
        </div>
            ...
            <div class="page-limit">
                <div class="myInnerWrap" v-loading="loading">
                    <template v-if="isNodata_msg" >
                        <NoData textMsg="您还没有任何商会消息！！！"></NoData>
                    </template>
                    <ul class="clearfix minH320">
                        <li class="blockHover" v-for="item in listData" v-bind:key="item.key">
                            ...
                            <div class="btns clearfix">
                                <a href="javascript:;" class="btnDanger rt" style="margin-left: 15px;" @click="agreeTissueship(item.userId,item.tissueId)">同意</a>
                                <a href="javascript:;" class="btnPlain rt" @click="refuseTissueship(item.userId,item.tissueId)">拒绝</a>
                            </div>
                        </li>
                    </ul>
...
<script>
    ...
    import {getMessageList,agreeTissueship,refuseTissueship} from '@/api/api';
    export default {
        data() {
            return {
                isTure: true,  
        ...
        methods: {
            ...
            getMessageList(pageNum) {
                let data = {
                    type:1,//  1：商会消息列表   2:诚邀消息列表 3.私密组消息
                    pageNum:pageNum,
                    pageSize:5
                }
            ...
            //同意
            agreeTissueship(userId,tissueId){
                let data = {
                    userId:userId,
                    tissueId:tissueId
                };
                agreeTissueship(data).then(res => {
                    if (res.code === 200) {
            ...
            //拒绝
            refuseTissueship(userId,tissueId){
                let data = {
                    userId:userId,
                    tissueId:tissueId
                };
                refuseTissueship(data).then(res => {
                    if (res.code === 200) {
                        this.$message.success('操作成功！');
                        this.getMessageList(1);
                    } else {
