...
<script>
    ...
    import WangEditor from 'wangeditor' //引入富文本编辑器
     import {richTextFileUpload,articlePost} from '@/api/api'
    export default {
        data() {
            return {
                name: 'editor',
        ...
        methods: {
            ...
            onSubmit() {
                ...
                articlePost(data).then(res => {
                    if (res.code === 200) {
                        this.$message.success('发布商讯成功！');
                        setTimeout(function(){
                            vm.$router.push('/index/myArticle');
                        },800);
                    } else {
                        this.$message({
                            message: res.msg,
