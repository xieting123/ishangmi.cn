<template>
    <div class="container">
        ...
        <div class="main layoutWrap clearfix" style="min-height: 76px;">
           <template>
                <BreadcrumbMult firstName="首页" secondName="名医" firstLink="/" secondLink='/doctorList' thirdName="发布文章"></BreadcrumbMult>
            </template>
        </div>
...
<script>
    ...
    import WangEditor from 'wangeditor' //引入富文本编辑器
     import {richTextFileUpload,publishBleacheryDoctor} from '@/api/api'
    export default {
        data() {
            return {
                name: 'editor',
        ...
        methods: {
            ...
            onSubmit(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        publishBleacheryDoctor(data).then(res => {
                            if (res.code === 200) {
