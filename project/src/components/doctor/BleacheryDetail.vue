<template>
    <div class="container">
        ...
        <div class="main layoutWrap clearfix minH60">
           <template>
                <BreadcrumbMult firstName="首页" secondName="文章" firstLink="/" secondLink="/doctorList" thirdName="文章详情"></BreadcrumbMult>
            </template>
        </div>
...
<script>
    ...
    export default {
        ...
        metaInfo() {
            return {
                title: '文章详情-'+this.shareTempData.title,
                htmlAttrs: {
                    lang: 'zh'
                },
            }
            ...
            bleacheryPraise(){
                ...
                bleacheryPraise(data).then(res => {
                    if (res.code === 200) {
                        ...
                        this.$router.push({
                            path: '/Login',
                            query: {redirect: '/doctorBleacheryDetail?bleacheryId='+this.bleacheryId} // 如果你使用钩子函数，your path 可以替换成to.fullPath
                        });
                    } 
                    else {
                        this.$message({
