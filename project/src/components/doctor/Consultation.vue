            ...
            <div class="main-content">
                ...
                <ul style="min-height: 300px;" class="clearfix" v-loading="loading">
                    <li class="business-road " v-for="item in listData" v-bind:key="item.key">
                        ...
                        <router-link class="linkTo" :to="{ name: 'DoctorConsultationDetail', query: {consultationId:item.id}}">
                       <div class="clearfix">
                            ...
                            <!-- <div class="shareAndManage clearfix rt">
                                ...
                                <a href="javascript:;" class="share_item" @click="shareInfo(item)">
...
<script>
    import {adviceDoctorList,adviceAnswerLike} from '@/api/api'
    ...
    export default {
        ...
        methods: {
            handleCurrentChange(val) {
                this.loading = true;
                this.adviceDoctorList(val);
            },
            adviceDoctorList(pageNum) {
                let data = {
                    pageNum:pageNum,
                    pageSize:4
                }
                adviceDoctorList(data).then(res => {
                    if (res.code === 200) {
            ...
            adviceAnswerLike(item){
                ...
                adviceAnswerLike(data).then(res => {
                    if (res.code === 200) {
                        ...
                        this.adviceDoctorList(1);
                    }else if(res.code === 10003){
                        this.$router.push({
                            path: '/Login',
                            query: {redirect: '/doctorList'} // 如果你使用钩子函数，your path 可以替换成to.fullPath
                        });
                    } 
                    else {
                        this.$message({
            ...
            shareInfo(item) {
                ...
                this.qrcodeData.value = item.shareUrl;
            },
        },
        created() {
