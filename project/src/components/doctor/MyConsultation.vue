<template>
    <div class="container">
        ...
        <div class="main layoutWrap clearfix shangdao">
            <template>
                <Breadcrumb firstName="首页" secondName="我的名医咨询" firstLink="/" ></Breadcrumb>
            </template> 
...
<script>
    import {myAdviceDoctorList,adviceAnswerLike} from '@/api/api'
    ...
    export default {
        ...
        methods: {
            handleCurrentChange(val) {
                this.loading = true;
                this.myAdviceDoctorList(val);
            },
            myAdviceDoctorList(pageNum) {
                let data = {
                    pageNum:pageNum,
                    pageSize:5
                }
                myAdviceDoctorList(data).then(res => {
                    if (res.code === 200) {
