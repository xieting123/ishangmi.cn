            ...
            <div class="main-content">
                ...
                <ul style="min-height: 300px;" class="clearfix" v-loading="loading">
                    <li class="business-road blockHover" v-for="item in listData" v-bind:key="item.key">
                        <router-link class="linkTo" :to="{ name: 'DoctorBleacheryDetail', query: {bleacheryId:item.id}}">
                       <div class="clearfix">
...
<script>
    import {bleacheryDoctorList} from '@/api/api'
    export default {
        data() {
            return {
                total:0,
        ...
        methods: {
            handleCurrentChange(val) {
                this.loading = true;
                this.bleacheryDoctorList(val);
            },
            bleacheryDoctorList(pageNum) {
                let data = {
                    pageNum:pageNum,
                    pageSize:4
                }
                bleacheryDoctorList(data).then(res => {
                    if (res.code === 200) {
